    ...
    html.split('\n').forEach(function (line, i) {
        ...
        if (spaces[0].length % 4 !== 0) {
            ...
            if (showMessage) {
                console.error(
                    '    Error. Line ' + (i + 1) +
                    '. The number of spaces in the indentation is not a multiple of four.'
                );
            }
        }
    });
    ...
    emptyElements.forEach(function (emptyElem) {
        ...
        if (pattern.test(html)) {
            ...
            if (showMessage !== false) {
                var msg = 'Closed single tag ' + emptyElem + '.';
...
exports.getBlockInsideInline = function (html, showMessage) {
    var msg = showMessage !== false ? 'Block tag {{prohibited}} inserted in inline tag {{elem}}.' : false;
...
exports.getBlockInsideP = function (html, showMessage) {
    var msg = showMessage !== false ? 'Block tag {{prohibited}} inserted in tag <p>.' : false;
...
exports.findImagesWithoutAlt = function (html, showMessage) {
    ...
    images.forEach(function (image) {
        ...
        if (!hasAlt) {
            if (showMessage !== false) {
                error(pattern, 'Image tag with missing or empty alt attribute.', html);
            }
